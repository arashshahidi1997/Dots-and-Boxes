...
class Agent:
    ...
    def __init__(self, w, h, experience='', alpha=0.5, gamma=0.8, epsilon=0.1, policy='e-greedy'):
        self.w = w
        self.h = h
        self.policy = policy
...
def random_opponent(theta, w, h):
    ...
    try:
        while True:
            ...
            if epsiode_counter % 10 == 0:
                if Delta < theta:
                    print('converged')
                    break

    except KeyboardInterrupt:
        np.save(folder + 'r-action_value_function.npy', p1.Q)
